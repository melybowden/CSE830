...
def plot_exp(fname, st=0, n=1000, sp=1, fig_title='Time for sorting N elements with insertion and merge sort', fldr='unsorted'):
    res = np.load('res/'+fname.split('/')[-1])
    res = np.array([res[_] for _ in range(st, n, sp)])
    intervals = [_ for _ in range(sp, n, sp)]
    fig, ax = plt.subplots()
    ax.plot(intervals, res[:N,0], 'bo', label='Insertion Sort')
    ax.plot(intervals, res[:N,1], 'ro', label='Merge Sort')
    ax.plot(intervals, res[:N,2], 'po', label='Tim Sort')
    ax.legend()
    ax.set(xlabel='N Inputs', ylabel='Time (s)',
            title=fig_title)
    ax.grid()
    plt.savefig('imgs/'+fldr+fname.split('.')[0]+'.png')
    plt.show()
...
####################### Q2 ##############################
START, N, STEP, N2, SORT, REVERSE = 0, 100, 1, 1000, False, False
PATH = '{}N_{}ST_{}SP_{}N2_{}SORT_{}R.npy'.format(N, START, STEP, N2, SORT, REVERSE)
exp1('res/'+PATH, st=START, n=N, sp=STEP, n2=N2, do_sort=SORT, do_reverse=REVERSE)
